...
sc = plt.scatter(turbX,turbY,c=PopAd,s=120,marker='v' ,edgecolor='black', linewidth='0.3', cmap=cm)
#plt.xlim(0,max(turbX))
#plt.ylim(0,max(turbY))
plt.clim(0.5,1.5)
plt.xlabel('x/D')
plt.ylabel('y/D')
plt.colorbar(sc, label='P_i / P_(Umet)',fraction=0.016, pad=0.04)
plt.axes().set_aspect('equal', 'datalim')
plt.grid()
plt.title('Openfoam, Dir: '+str(Dir)+',Umet: '+str(Um))
plt.savefig('rawsonPotenciaOpenfoam'+str(Dir), dpi = 300)
plt.show
plt.clf
...
plt.figure(6)
cm = plt.cm.get_cmap('bwr')
sc = plt.scatter(turbX,turbY,c=PmlistAd,s=120,marker='v' ,edgecolor='black', linewidth='0.3', cmap=cm)
#plt.xlim(0,max(turbX))
#plt.ylim(0,max(turbY))
plt.clim(0.5,1.5)
plt.xlabel(r'$\alpha$x/D')
plt.ylabel('ldfkjgklsdfgkjly/D')
plt.colorbar(sc, label=r'$P_i / P_{u_{met}}$',fraction=0.016, pad=0.04)
plt.axes().set_aspect('equal', 'datalim')
plt.grid()
plt.title('Measurements, Dir: '+str(Dir)+',Umet: '+str(Um))
plt.savefig('rawsonPotenciaMedidas'+str(Dir), dpi = 300)
plt.show
plt.clf

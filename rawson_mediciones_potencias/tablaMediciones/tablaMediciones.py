...
def superficieSuavizada (Umc,Dirmc,Pmc ):
    ...
    blurred_face = ndimage.gaussian_filter(grid_z0, sigma=1) #  INTERPOLACION ESPACIAL ----------------------------------------------
    #giro la iamgen
...
#------PARA TODAS LAS TURBINAS Y EL PARQUE ENTERO
turbinasLista=np.arange(1, 45, 1) #la 44 es el parque entero
Pturbinas=[]
for turbina in turbinasLista:
    print('turbina: '+str(turbina))
    #-----EXTRAIGO LA U, DIR Y POTENCIA MEDIDAS EN EL TIEMPO
    Umed,Dirmed,Pmed,horas,dias1 = medicionesExtraccion(mediciones,turbina)

    #-----GRAFICO LOS DATOS MEDIDOS COMO NUBE DE PUNTOS PARA CADA TURBINA
...
for u in range (len(Umc)):
    for d in range (len (Dirmc)):
        ...
        for t in range(43):
            Pt.append(0)
	for t in range(len(turbinasLista)-1): #porque el ultimo valor es el parque entero
            #print(Pt[t])
	    #print(Pturbinas[t][u][d])
	    Pt[t]=Pturbinas[t][u][d]
        #potencia del parque entero
        Ptotal=Pturbinas[len(Pturbinas)-1][u][d]
